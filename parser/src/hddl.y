...
%token KEY_AND KEY_OR KEY_NOT KEY_IMPLY KEY_FORALL KEY_EXISTS KEY_WHEN KEY_INCREASE KEY_TYPEOF KEY_XOR
...
task_def : '(' task_or_action NAME
			...
			precondition_option
			nd-effects ')'{
				auto n_effects = $6->size();
				// check whether the action has non deterministic effects or not
				if (n_effects == 1) {
					parsed_task d_task {$3, $4, $5, $6->at(0)};
					if ($2) parsed_abstract.push_back(d_task); else parsed_primitive.push_back(d_task);
				}
				else {
					// add an abstract task that can be refined to its effects
					auto empty_formula = new general_formula();
					empty_formula->type = EMPTY;
					parsed_task nd_task{ $3, $4, $5, empty_formula};
					parsed_abstract.push_back(nd_task);
					// create primitive tasks for each set of effects,
					// and their corresponding methods
					var_and_const* args = new var_and_const;
					for (const auto i: $4->vars) {
						args->vars.push_back(i.first);
					}
					for (size_t i=0; i < n_effects; i++){
						...
						empty_formula->type = EMPTY;
						m.eff = empty_formula;
						// method subtask construction
						sub_task* st = new sub_task();
						st->id = "__t_id_" + to_string(task_id_counter);
						task_id_counter++;
						st->task = new_name;
						st->arguments = args;
						std::vector<sub_task*> m_tasks;
						m_tasks.push_back(st);
						auto task_net = new parsed_task_network;
						task_net->tasks = m_tasks;
						task_net->constraint = empty_formula;
						m.tn = task_net;
						parsed_methods[$3].push_back(m);
					}		
				}
			}
